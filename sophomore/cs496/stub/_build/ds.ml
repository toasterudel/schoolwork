...
and
  ...
  | ExtendEnvRec of string*(string list)*Ast.expr*env
...
let rec apply_env (env:env) (id:string):exp_val option =
  ...
  | ExtendEnvRec(key,params,body,en) ->
    if id=key
    then Some (ProcVal(params,body,env))
...
let rec string_of_expval = function
  ...
  | ProcVal (ids,body,env) -> "ProcVal ("^(Ast.string_of_string_list ids)^","^Ast.string_of_expr body^","^ string_of_env env^")"
...
and
  ...
  | ExtendEnvRec(id,params,body,env) -> "("^id^","^(Ast.string_of_string_list params)^","^Ast.string_of_expr body^")"^string_of_env env

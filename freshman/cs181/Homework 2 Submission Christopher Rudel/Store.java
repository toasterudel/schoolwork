...
public class Store {
    ...
    public Book produceBook( String line ) {
    	...
    	Book answer = null;
    	ArrayList<Integer> underscores = new ArrayList<Integer>();
        for(int i=0; i<line.length(); i++)
        {
        	if(line.charAt(i) == '_')
        		underscores.add(i);
        }
        String title = line.substring(0, underscores.get(0));
        String author = line.substring(underscores.get(0)+1, underscores.get(1));
    	if(underscores.size()==2)
    	{
    		int cost = Integer.parseInt(line.substring(underscores.get(1)+1));
    	    answer = new PaperbackBook(title, author, cost);
    		
    	}
    	else if(underscores.size()==3)
    	{
    		int cost = Integer.parseInt(line.substring(underscores.get(1)+1 , underscores.get(2)));
    		if(line.charAt(underscores.get(2)+1) == 'l')
    			answer = new HardcoverBook(title, author, cost, "Leather");
    		else if(line.charAt(underscores.get(2)+1) == 'c')
    			answer = new HardcoverBook(title, author, cost, "Cloth");
    		else if(Character.isDigit(line.charAt(underscores.get(2)+1))  ) 
    				answer = new AudioBook(title, author, cost, Integer.parseInt(line.substring(underscores.get(2)+1)));
    		else if(line.charAt(underscores.get(2)+1) == 'h')
    			answer = new ElectronicBook(title, author, cost, line.substring(underscores.get(2)+1));

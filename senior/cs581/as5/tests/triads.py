...
self_loop_count, pos_count, neg_count, G = graph_and_stats(
    "epinions_small.csv")
...
print("NUMBER OF NODES IN TRIADS:", len(
    set([val for sublist in triads for val in sublist])))
...
triad_table = table.sort_values(
    ['trust_category'], ascending=False).reset_index(drop=True)
...
print("Expected distribution of TTT, TTD, TDD, and DDD triads are:\n{}, {}, {}, and {}. ".format(*list("n={0}: p={1:.2f}%".format(i, x*100) for i, x in zip((len(triad_table[triad_table['trust_category'] == 'TTT']),    len(
    triad_table[triad_table['trust_category'] == 'TTD']),    len(triad_table[triad_table['trust_category'] == 'TDD']),    len(triad_table[triad_table['trust_category'] == 'DDD'])), (p_type_1, p_type_2, p_type_3, p_type_4)))))
...
print("Actual distribution of TTT, TTD, TDD, and DDD triads are respectively {0:.2f}%, {1:.2f}%, {2:.2f}%, and {3:.2f}%.".format(
    *list(map(lambda x: x*100, (type_1 / n_triads, type_2 / n_triads, type_3 / n_triads, type_4 / n_triads)))))
...
print("NUMBER OF EDGES IN MINIMUM SPANNING TREE:", len(
    nx.minimum_spanning_tree(G).edges()))  # edge size of minimum spanning tree
